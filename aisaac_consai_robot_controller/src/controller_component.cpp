...
#include "aisaac_consai_robot_controller/controller_component.hpp"
#include "aisaac_consai_robot_controller/geometry_tools.hpp"
#include "aisaac_consai_robot_controller/control_tools.hpp"
...
namespace aisaac_consai_robot_controller
{
      ...
      : Node("controller", options)
  {
    ...
    parser_.param_threshold_looking_ball_theta = get_parameter(
                                                     PARAM_THRESHOLD_LOOKING_BALL_THETA)
                                                     .get_value<double>();
    ...
    for (int i = 0; i < ROBOT_NUM; i++)
    {
      ...
      timer_pub_control_command_.push_back(
          create_wall_timer(
              10ms, [this, robot_id = i]()
              { this->on_timer_pub_control_command(robot_id); }));
      timer_pub_control_command_[i]->cancel(); // タイマーを停止
      timer_pub_stop_command_.push_back(
          create_wall_timer(
              100ms, [this, robot_id = i]()
              { this->on_timer_pub_stop_command(robot_id); }));
    ...
    // add by shimizu
    pub_aisaac_strategy_msg_ = create_publisher<aisaac_msgs::msg::AisaacStrategy>("aisaac_strategy", 10);
    //
  ...
  /// @brief pack msg. Add by shimizu
  /// @param robot_id
  /// @param goal Action
  /// @param kick_power
  /// @param goal_pose
  /// @param msg Output
  void Controller::pack_aisaac_strategy(const unsigned int robot_id,
                                        ...
                                        aisaac_msgs::msg::AisaacStrategy &msg)
  {
    ...
    //// [TODO] 状況によって変えるべきだが一旦は固定値
    msg.robot_position_init = false;
    ...
    double kick_distance = 3.; // [TODO]現在適当な値
    msg.target_ball.x = goal_pose.x + kick_distance * cosf(goal_pose.theta);
  ...
  void Controller::on_timer_pub_control_command(const unsigned int robot_id)
  {
            ...
            dribble_power))
    {
      ...
      /// add by shimizu
      aisaac_msgs::msg::AisaacStrategy output_msg;
      pack_aisaac_strategy(robot_id, dribble_power, kick_power, goal_pose, output_msg);
      pub_aisaac_strategy_msg_->publish(std::move(output_msg));
      ///
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(aisaac_consai_robot_controller::Controller)
